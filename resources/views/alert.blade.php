...
<div @alpine-show-message.window="dispatch2($event.detail.data, $event.detail.type)" x-init="@if(session('message')) dispatch2('{{ session('message') }}', @if(session('type')) this.type = '{{ session('type') }}'; @endif) @endif" x-data="{ timeout: false, openNotify: false, open1: false, data: '', type: 'success', dispatch2(data, type = 'success') {
