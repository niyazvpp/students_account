...
<div class="px-8 py-4 my-6" x-data>
    ...
    <div class="py-6 flex justify-end items-center">
        <button @click="$dispatch('adduser')" type="button" class="btn btn-blue px-8 py-1.5 my-auto w-max">New Teacher</button>
    </div>
   ...
   <div class="w-full">
        ...
        $headers = ['Id', 'Name', 'Username', 'Email', 'Mobile', 'Old Balance'];
        $body = [];
        foreach ($teachers as $count => $user) {
            $body[] = [
                $count * 1 + 1,
                $user->name,
                $user->username,
...
x-show="open" @adduser.window="()=>{open = true}" class="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
