...
<div class="" x-data>
    ...
    <div class="py-6 flex sm:justify-end justify-center items-center">
        <button @click="$dispatch('adduser', 0)" type="button" class="btn btn-yellow px-8 py-1.5 my-auto w-max">New Category</button>
    </div>
   ...
   <div class="w-full">
        ...
        foreach ($categories as $count => $category) {
            $body[] = [
                ...
                $category->name,
                $category->slug,
                '<button @click="$dispatch(\'adduser\', ' . $category->id . ')" class="btn text-gray-600 bg-gray-100 border border-gray-200 shadow-none">Edit</button>'
            ];
        }
    ...
    >
      ...
      <div class="relative inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl sm:w-full"
        ...
        <div class="bg-white form-container px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <form @submit.prevent="submit($event.target)" action="{{ route('categories.edit') }}" method="POST">
            @csrf
            <input type="hidden" name="category" id="category" :value="currentClass.id">
            <h3 class="text-lg leading-6 mb-4 font-semibold text-gray-900" x-text="add ? 'Add New Category' : 'Edit Category'">
              Add New Category
            </h3>
        ...
        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
          <button :disabled="loading" :class="{ 'hover:bg-blue-500 focus:ring-2 focus:ring-offset-2 focus:ring-blue-400': !loading, 'opacity-40': loading }" x-text="loading ? 'Loading...' : add ? 'Add Category' : 'Save Changes'" @click.prevent="submit($event.target.closest('.modal').querySelector('.form-container form'))" type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2  text-base font-medium text-white bg-blue-400 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm">Add Category</button>
  ...
  <script type="text/javascript">
    function modal() {
      return {
        ...
        show(id = 0) {
          this.open = true;
          var filtered = this.categories.filter(classe => classe.id == id);
